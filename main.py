...
genai.configure(api_key="AIzaSyDUFDzjEouaorcGzMGjwn_9_-zvqI3Eflo")  # Replace with your actual Gemini API key
...
def send_message():
    ...
    threading.Thread(target=play_sound, args=(temp_file,), daemon=True).start()

# Send button to send message
send_button = ttk.Button(input_frame, text="Send", command=send_message)
send_button.pack(side=tk.RIGHT, padx=5)

# Initialize TTS engine with human-like voice
engine = pyttsx3.init()

# Set properties for natural sounding voice
voices = engine.getProperty('voices')
engine.setProperty('voice', voices[1].id)  # Adjust the index to choose a different voice
...
def capture_emotion():
    ...
    message = emotion_messages.get(detected_emotion, "Emotion not recognized.")
    chat_log.config(state='normal')
